...
export class AuthService {
    ...
    async register (payload: Required<UserRegisterDto>) {
        ...
        const token = await this.jwt.sign({id: data.id})
    ...
    async login(payload: Required<UserLoginDto>) {
        ...
        if (!deshif) throw new NotFoundError('Invalid username or password')

        const token = await this.jwt.sign({ id: user.id, username: user.username })
